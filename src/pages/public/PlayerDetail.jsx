  ...
  return (
    <div className="relative w-[160px] mx-auto">
      <svg viewBox="0 0 160 160" width="160" height="160">
        ...
        {gridPts.map(([x, y], i) => (
          <line key={i} x1={cx} y1={cy} x2={x.toFixed(1)} y2={y.toFixed(1)} stroke="rgba(255,255,255,0.07)" strokeWidth={1} />
        ))}
        <path d={toPath(pts)} fill="rgba(16,185,129,0.15)" stroke="#10b981" strokeWidth={1.5} />
        {pts.map(([x, y], i) => (
          <circle key={i} cx={x.toFixed(1)} cy={y.toFixed(1)} r={2.5} fill="#10b981" opacity={0.9} />
        ))}
      </svg>
...
function PctBar({ pct }) {
  ...
  return (
    <div className="h-[3px] bg-white/[0.1] rounded-full mt-1.5 overflow-hidden">
      <div className="bg-[#10b981] h-full rounded-full transition-all duration-500" style={{ width: `${pct}%` }} />
    </div>
  )
}
...
export default function PlayerDetail() {
  ...
  if (!player) return (
    <div className="max-w-3xl mx-auto px-4 py-12 text-center">
      ...
      <Link to="/" className="text-[#10b981] font-medium">← Back to dashboard</Link>
    </div>
  )
    ...
    <div className="max-w-3xl mx-auto px-4 pb-12">
            ...
            {showTopUp ? (
              ...
              <div className="flex items-center justify-between">
                ...
                <button
                  ...
                  className="text-sm text-[#10b981] font-medium hover:underline flex items-center gap-1"
                >
      ...
      {completedWeeks.length > 0 && (
        <div className="grid grid-cols-3 gap-3 mb-4">
              ...
              <>
                <svg viewBox={`0 0 ${heatWeeks.length * 10} 10`} width="100%" className="mt-0.5 mb-1">
                  {heatWeeks.map((w, i) => {
                    ...
                    const fill = !attended ? 'rgba(255,255,255,0.08)'
                      : runs >= 30 ? '#10b981'
                      : runs >= 10 ? '#6ECBAD'
                      : '#B8E4D6'
          ...
          {player.type !== 'ppm' ? (
            <div className="card text-center py-3 px-2">
              <div className={`text-[24px] font-medium tabular-nums ${matchesLeft <= 2 ? 'text-red-400' : matchesLeft <= 5 ? 'text-amber-400' : 'text-[#10b981]'}`}>
                ~{matchesLeft}
              </div>
      ...
      {perfs.length > 0 && (
        <div className="card mb-4">
          ...
          {/* Radar chart */}
          {radarScores && (
            <div className="mb-4 pt-2 pb-3 border-t border-white/[0.05]">
