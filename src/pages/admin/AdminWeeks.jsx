...
export default function AdminWeeks() {
  ...
  // result editor per row
  const [editResult, setEditResult] = useState(null) // week_id
  ...
  function openAttendance(weekId) {
    const init = Object.fromEntries(
      records.filter(r => r.week_id === weekId).map(r => [r.player_id, r.status])
    )
  ...
  // Auto-calculate match fee from total cost + expected players
  ...
  async function saveAttendance(week) {
    ...
    try {
      ...
      if (cfg?.auto_deduct_on_sync && !alreadyDeducted) {
        ...
        showToast(alreadyDeducted ? 'Attendance updated (deductions already applied for this week)' : 'Attendance saved')
      }
      setSelected(null)
    } catch (e) {
      showToast(e.message, 'error')
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b border-gray-200">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase">Venue / Teams</th>
          ...
          <tbody className="divide-y divide-gray-100">
            {weeks.map(w => {
              ...
              return (
                <tr
                  key={w.week_id}
                  className={`hover:bg-gray-50 ${w.status === 'completed' ? 'cursor-pointer' : 'cursor-default'}`}
                  onClick={() => w.status === 'completed' && setDetail(w.week_id)}
                >
                  <td className="px-4 py-3 font-medium text-gray-800">
                    {format(parseISO(w.match_date), 'MMM d, yyyy')}
                    {w.result && <div className="text-xs text-green-600 font-medium">{w.result}</div>}
                  </td>
        ...
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-2 flex gap-2">
              <button onClick={() => setAttendanceMap(Object.fromEntries(players.map(p => [p.id, 'played'])))} className="btn-secondary text-xs py-1">Mark All Present</button>
              <button onClick={() => setAttendanceMap(Object.fromEntries(players.map(p => [p.id, 'absent'])))} className="btn-secondary text-xs py-1">Mark All Absent</button>
            </div>
